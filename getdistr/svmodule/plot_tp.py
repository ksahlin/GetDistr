...
def plot2(x_y_ins_true, x_y_ins_false, x_y_del_true, x_y_del_false):
	...
	print tot_fps
	sd1, = ax.plot(x_del_true[:6], y_del_true[:6],'^-b',label='$FP_{del}$,$\sigma$=25')
	sd1, = ax.plot(x_ins_true[:6], y_ins_true[:6],'o--b',label='$FP_{ins}$,$\sigma$=25')
	sd2, = ax.plot(x_del_true[6:12], y_del_true[6:12],'^-r',label='$FP_{del}$,$\sigma$=50')
	sd2, = ax.plot(x_ins_true[6:12], y_ins_true[6:12],'o--r',label='$FP_{ins}$,$\sigma$=50')
	sd3, = ax.plot(x_del_true[12:], y_del_true[12:],'^-g',label='$FP_{del}$,$\sigma$=75')
	sd3, = ax.plot(x_ins_true[12:], y_ins_true[12:],'o--g',label='$FP_{ins}$,$\sigma$=75')
	...
	#plt.show()
	plt.savefig('/Users/ksahlin/Dropbox/GetDist/paper-Asmblthn-contest/figures/Clever_true_400.pdf', format='pdf')

	fig, ax = plt.subplots()
	fig.suptitle('$H_0$, $\mu=400$')
	plt.xlabel('indel size')
	plt.ylabel('# detected')	
	...
	#plt.show()
	plt.savefig('/Users/ksahlin/Dropbox/GetDist/paper-Asmblthn-contest/figures/Clever_false_400.pdf', format='pdf')
