...
def parse_bam(bam_file,param):
	...
	with pysam.Samfile(bam_file, 'rb') as bam:
		...
		print >>  stats_file, 'True lower 0.025 quantile distance from mean:{0}'.format(param.adjusted_mu - scanner.ecdf.means[lower_index])
		print >>  stats_file, 'True upper  0.025 quantile distance from mean:{0}'.format(scanner.ecdf.means[upper_index] - param.adjusted_mu)
